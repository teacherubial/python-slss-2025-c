...
if __name__ == "__main__":
	...
	# Use the methods
	human_one.talk()
	print(f{human_one.name}'s thought: ", human_one.think())
...
> Jim's thought: What's the meaning of life.
```

Notice the `self` parameter in the methods that we created. `self`, at first, can be a strange concept. It represents the object itself. It's a way to represent the specific instance of the class.

If you come from Java, `self` is Python's version of `this`.

I think this [video](https://www.youtube.com/watch?v=yYALsys-P_w) might be helpful in explaining `self`.
